...
class Multimodal_Datasets(Dataset):
    def __init__(self, dataset_path, data='Husformer', split_type='train'):
        ...
        # Map modalities to the actual keys in the pickle file:
        # eeg -> modality_1, gsr -> modality_2, bvp -> modality_3
        self.m1 = torch.tensor(dataset[split_type]['eeg'].astype(np.float32)).cpu().detach()
        self.m2 = torch.tensor(dataset[split_type]['gsr'].astype(np.float32)).cpu().detach()
        self.m3 = torch.tensor(dataset[split_type]['bvp'].astype(np.float32)).cpu().detach()
